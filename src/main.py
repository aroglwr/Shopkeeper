...
async def osu(interaction: discord.Interaction):
    await interaction.response.defer()
    embed=discord.Embed(title=f'osu! is not supported', description=f'Please use another bot for all of your osu! needs such as:', color=0xff66aa)
    embed.thumbnail(url="https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Osu%21_Logo_2016.svg/2048px-Osu%21_Logo_2016.svg.png")
    ...
    disp_name = str(client.user)[:-5] + " bot"
    embed.set_footer(text=f'play taiko\n{disp_name}')
    await interaction.followup.send(embed=embed)
# Steam Commands
...
async def gameanalysis(interaction: discord.Interaction, summoner_name: str, region: str, game_number: int = 1):
    ...
    league.drawMap(riot_token, match_id, region_f, mapType, summonerData[1]) # https://europe.api.riotgames.com/lol/match/v5/matches/EUW1_6579462123/timeline?api_key=RGAPI-b560a406-267a-4dfe-b7a8-c14b5c6f2a50
...
async def masteryprofile(interaction: discord.Interaction, summoner_name: str, region: str):
    ...
    summonerData = league.getSummonerData(summoner_name, region, riot_token)
    masteryData = league.masteryGraph(riot_token, summonerData[5], region)
    level_list = masteryData[3]
    highest = masteryData[0]
    embed = discord.Embed(title="mastery info", description="this command is incomplete")
...
@tree.command(name = "kagserverlist", description="Gets King Arthur's Gold Server list")
async def kagserverlist(interaction: discord.Interaction, official_only: bool = False):
    """kagserverlist description
    ...
    apiData = kag.getWebStatus()
    servers = kag.getServerList(official_only)
    serverList = servers[0][:5]
    if official_only:
        desc_text = f"There are currently {servers[1]} players on official servers"
    else:
        desc_text = f"There are currently {servers[1]} players online"
    embed=discord.Embed(title="View on KAGstats.com", url="https://kagstats.com/#/servers", description=f"{desc_text}\nLast updated <t:{int(time.time())}:R>", color=0xFFD700) # f"Last updated {general.dateConvert(time.time())[1][:-1]}"
...
async def kagclans(interaction: discord.Interaction, search: str = ""):
    ...
    Args:
        name (str): Name of clan to search (leave blank for a list of top clans)
    """
    await interaction.response.defer()
    ...
    if len(search) != 0:
        embed=discord.Embed(title="View on KAGstats.com", url="https://kagstats.com/#/clans", description=f"server stats here", color=0xFFD700)
        #embed.set_thumbnail("https://kagstats.com/favicon.ico")
